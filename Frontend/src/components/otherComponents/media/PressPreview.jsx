...
const PressPreview = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <span className="text-lg text-gray-500">Loading press content...</span>
      </div>
    );
  }
  ...
  if (!press) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <span className="text-lg text-gray-500">Press content not found.</span>
      </div>
    );
  }

  // Determine if content is a YouTube URL or an image
  const isYoutube = isYoutubeUrl(press.content);
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="text-gray-500 text-center">
              Uploaded: {new Date(press.createdAt).toLocaleDateString()}
            </div>
          </div>
        </div>
      </div>
